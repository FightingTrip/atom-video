...
export const useVideoStore = defineStore(
  ...
  () => {
    // 状态
    const videos = ref<ExtendedVideo[]>([]);
    ...
    // 用于存储过滤后的视频列表
    const filteredResults = ref<ExtendedVideo[]>([]);
    ...
    const fetchVideos = async () => {
      ...
      try {
        ...
        // 确保每个视频都有category属性
        const videosWithCategory = response.videos.map(video => ({
          ...video,
          category: video.tags[0] || 'general', // 使用第一个标签作为分类
        }));

        videos.value =
          currentPage.value === 1 ? videosWithCategory : [...videos.value, ...videosWithCategory];
    ...
    const searchVideos = async (query: string) => {
      ...
      try {
        ...
        // 添加category属性
        const videosWithCategory = filteredResults.map(video => ({
          ...video,
          category: video.tags[0] || 'general', // 使用第一个标签作为分类
        }));

        videos.value = videosWithCategory;
  ...
  {
    persist: {
      key: 'atom-video-user-data',
      storage: localStorage,
      paths: ['watchHistory', 'likedVideos', 'savedVideos'],
    },
  }
);
