...
export const useVideoStore = defineStore('video', () => {
    ...
    const filteredVideos = computed(() => {
        ...
        if (searchQuery.value) {
            ...
            results = results.filter(video => video.title.toLowerCase().includes(query) ||
        ...
        const sortMap = {
            relevance: (a, b) => {
                const queryInTitle = (v) => v.title.toLowerCase().includes(searchQuery.value.toLowerCase());
    ...
    const searchVideos = async (query) => {
        ...
        try {
            ...
            // 在本地进行搜索过滤
            const filteredResults = response.videos.filter(video => video.title.toLowerCase().includes(query.toLowerCase()) ||
    ...
    const getVideoComments = async (videoId, offset = 0) => {
        try {
    ...
    const toggleVideoFavorite = async (videoId) => {
        try {
    ...
    const toggleAuthorFollow = async (authorId) => {
        try {
    ...
    const replyComment = async (videoId, commentId, content, replyToId) => {
        try {
    ...
    // 获取视频列表
    const getVideos = async (page = 1, category) => {
        try {
            loading.value = true;
            error.value = null;
    ...
    // 获取推荐视频
    const getRecommendedVideos = async (page = 1) => {
        try {
            loading.value = true;
            error.value = null;
    ...
    // 获取收藏视频
    const getLibraryVideos = async (page = 1) => {
        try {
            loading.value = true;
            error.value = null;
