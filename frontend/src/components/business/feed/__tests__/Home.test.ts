...
describe('Home', () => {
  ...
  it('应该正确渲染所有区域', () => {
    ...
    expect(recommendedSection.findComponent({ name: 'VideoGrid' }).props('videos')).toEqual(
    ...
    expect(trendingSection.findComponent({ name: 'VideoGrid' }).props('videos')).toEqual(
      mockTrendingVideos
    );
    ...
    expect(latestSection.findComponent({ name: 'VideoGrid' }).props('videos')).toEqual(
      mockLatestVideos
    );
  });
